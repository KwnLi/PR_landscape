...
activeCat(lf.evt) <- "EVT_NAME"  # set the active category of the raster

# as.numeric turns the raster into a numeric layer based on the active category set above
lf.evt.num <- as.numeric(lf)
...
rcl.evt.mat <- reclass.evt %>% dplyr::select(evt.num, reclass.num) %>% as.matrix()
...
areas.df <- areas.df %>% rename(!!!reclasses)

write.csv(areas.df, "landcover_buffers.csv", row.names = FALSE)
```
...
ring.areas.df <- bind_rows(ring.areas, .id = "outer.radius") %>% 
  replace_na(list(frac_9=0, frac_6=0)) %>%
  relocate(farm,inner.radius) %>% rename(!!!reclasses)

write.csv(ring.areas.df, "landscape_rings.csv", row.names = FALSE)
...
write.csv(allcov.areas, "veg_cov_buffers.csv", row.names = FALSE)
...
write.csv(allcov.rings, "veg_cov_rings.csv", row.names = FALSE)
